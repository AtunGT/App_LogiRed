package com.arthur.gloria.logired.features.login.registerclient.data.repository
...
class RegisterClientRepositoryImpl @Inject constructor(
    private val api: ApiService,
    @ApplicationContext private val context: Context
) : RegisterClientRepository {

    override suspend fun register(client: ClientData): Result<Unit> {
        return try {
            val imagePart = client.imageUri?.let { uri ->
            ...
            val response = api.createUser(
                name        = client.name.toTextRequestBody(),
                lastname    = client.lastname.toTextRequestBody(),
                email       = client.email.toTextRequestBody(),
                numberphone = client.numberPhone.toTextRequestBody(),
                birthdate   = client.birthdate.toTextRequestBody(),
                password    = client.password.toTextRequestBody(),
                userType    = "1".toTextRequestBody(),
                image       = imagePart
            )
